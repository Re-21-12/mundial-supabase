...
@Component({
  ...
  template: `
    <div class="tc-card" [class.tc-card--joined]="league().is_joined">
      ...
      <div class="tc-banner" [style.background]="bannerBackground()">
        @if (league().logo_url) {
          <img class="tc-banner__img" [src]="league().logo_url" [alt]="league().name" loading="lazy" />
        } @else {
  ...
  styles: [`
    ...
    .tc-metric i { font-size: 0.75rem; color: var(--muted-foreground); }
    .tc-metric__label { font-size: 0.7rem; font-weight: 400; color: var(--muted-foreground); }
    .tc-metric--prize { color: #d4a017; }
    .tc-metric--prize i { color: #d4a017; }
    ...
    .tc-user-pos { font-size: 1.2rem; line-height: 1; flex-shrink: 0; }
    ...
    .tc-user-pts { font-size: 0.9rem; font-weight: 900; color: var(--primary); }
    .tc-pts-label { font-size: 0.7rem; font-weight: 400; color: var(--muted-foreground); }
    .tc-user-rank { font-size: 0.7rem; color: var(--muted-foreground); margin-left: auto; }

    /* Footer */
    .tc-footer { padding: 0 1rem 1rem; }
    ...
    .tc-btn--primary:hover { opacity: 0.88; transform: translateY(-1px); }
...
export class LeagueTournamentCardComponent {
  ...
  protected readonly bannerInitial = computed(() =>
    (this.league().name[0] ?? '?').toUpperCase(),
  );
  ...
  protected readonly statusLabel = computed(() => {
    switch (this.league().status) {
      ...
      case 'inactive': return 'Pausada';
      case 'finished': return 'Finalizada';
      default: return this.league().status ?? 'Sin estado';
    }
  });

  protected readonly statusChipClass = computed(() => {
    switch (this.league().status) {
      case 'active': return 'tc-chip tc-chip--active';
      case 'draft': return 'tc-chip tc-chip--draft';
      case 'inactive': return 'tc-chip tc-chip--inactive';
      case 'finished': return 'tc-chip tc-chip--finished';
      default: return 'tc-chip tc-chip--inactive';
    }
  });
  ...
  protected readonly joinLabel = computed(() => {
    ...
    return amt > 0 ? `Unirse · $${amt} MXN` : 'Unirse gratis';
  });
