...
import { formFields } from './user-session-form';
...
export class UserSessionPage implements OnInit {
  ...
  updateData = async (data: Partial<Database['public']['Tables']['USER_SESSION']['Update']>) => {
    const response = await this.dynamicService.updateData('USER_SESSION', data, {
  ...
  deleteData = async (rowId: string) => {
    ...
    const response = await this.dynamicService.deleteData('USER_SESSION', {
      field: 'user_session_id',
      value: rowId,
    });
