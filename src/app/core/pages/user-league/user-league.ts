...
export class UserLeaguePage implements OnInit {
  ...
  updateData = async (data: Partial<Database['public']['Tables']['USER_LEAGUE']['Update']>) => {
    const response = await this.dynamicService.updateData('USER_LEAGUE', data, {
      field: 'user_league_id',
      value: this.id()!,
    });
    return response;
  };
  deleteData = async (rowId: string) => {
    ...
    const response = await this.dynamicService.deleteData('USER_LEAGUE', {
      field: 'user_league_id',
      value: rowId,
    });
