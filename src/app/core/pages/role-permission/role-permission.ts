...
interface UserRow { user_id: number; name: string; email: string; }
interface RoleRow { role_id: number; name: string; }
interface PermRow { permission_id: number; name: string; description: string; }
interface RolePermRow { role_permission_id: number; role_id: number; permission_id: number; }
interface UserRoleRow { user_role_id: number; user_id: number; role_id: number; }
interface PermGroup { key: string; perms: (PermRow & { action: string })[]; }
...
export class RolePermissionPage implements OnInit {
  ...
  async toggleRolePerm(roleId: number, permId: number, checked: boolean): Promise<void> {
    ...
    if (checked) {
      ...
      const rp = this.rolePerms().find(
        (r) => r.role_id === roleId && r.permission_id === permId,
      );
      if (rp) {
        const { error } = await this.db.client
          .from('ROLE_PERMISSION')
