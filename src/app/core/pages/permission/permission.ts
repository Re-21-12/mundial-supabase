...
import { formFields } from './permission-form';
...
export class PermissionPage implements OnInit {
  ...
  getData = async () => {
    ...
    if (this.id()) {
      response = await this.dynamicService.fetchData({
        ...
        columns: '*',
        filters: { field: 'permission_id', value: this.id()! },
      });
    } else {
      response = await this.dynamicService.fetchData({
        table: 'PERMISSION',
  ...
  updateData = async (data: Partial<Database['public']['Tables']['PERMISSION']['Update']>) => {
    const response = await this.dynamicService.updateData('PERMISSION', data, {
      field: 'permission_id',
      value: this.id()!,
    });
    return response;
  ...
  deleteData = async (rowId: string) => {
    ...
    const response = await this.dynamicService.deleteData('PERMISSION', {
      field: 'permission_id',
      value: rowId,
    });
