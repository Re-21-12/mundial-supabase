...
import { WalletService, WalletSummary } from '../../admin/wallet/wallet.service';
...
export class WalletTopupPage implements OnInit {
  ...
  async ngOnInit() {
    ...
    const depositEntry =
      items.find(
        (c) =>
          c.description.toLowerCase().includes('depósito') ||
          c.description.toLowerCase().includes('deposito'),
      ) ?? items[0];
  ...
  private buildFields(items: CatalogItem[], defaultTxId: number): FieldBase<any>[] {
    const fields: FieldBase<any>[] = [
      {
        ...
        label: 'Monto a cargar (GTQ)',
        placeholder: '0',
        hint: 'Monto mínimo: Q50',
        value: 0,
    ...
    if (items.length > 0) {
      fields.push({
        ...
        hint: '',
        value: defaultTxId || items[0].catalog_id,
        rules: [Validators.required],
  ...
  protected async onFormSubmit(payload: string) {
    ...
    const w = this.wallet();
    if (!w) {
      this.errorMsg.set('No se encontró tu wallet.');
      return;
    }
    ...
    const { error } = await this.walletSvc.deposit(
      w.wallet_id,
      this.userId,
      amount,
      Number(txType),
